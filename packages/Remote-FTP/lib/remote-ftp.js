var FS, Path, $, Client, TreeView, Ignore, AddDialog, MoveDialog, NavigateTo;
	...
	activate: function (state) {
		...
		atom.commands.add('atom-workspace', {
			...
			'remote-ftp:create-ftp-config': function () {
				...
				FS.writeFile(atom.project.getDirectories()[0].resolve('.ftpconfig'), JSON.stringify({
					"protocol": "ftp",
					"host": "example.com",
					"port": 21,
					"user": "user",
			...
			'remote-ftp:download-selected-local': function () {
				...
				if ( confirm("downloading selected local") ) {
